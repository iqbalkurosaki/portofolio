	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$(document).on('submit', '#uploadimage', function(e){
				...
				formData.append('arrayARN', JSON.stringify(arrName))
		        $.ajax({
		            url : 'ajal.php',
		            data : formData,
		            type : 'POST',
...
?>
								<select class="mx-1 form-control underline" name="<?php echo $res['nama']; ?>" id="role-responden-<?php echo $res['nama']; ?>" required="">
...
?>
						...
						<div class="form-group pengisian row justify-content-end mb-5">
							...
							<input class="col-sm-3 form-control form-control-plaintext underline" type="date" name="batasWaktu">
						</div>
		  				<div class="form-group">
		  					<textarea class="form-control-plaintext pl-3 underline" placeholder="Judul" name="judul"></textarea>
						</div>
						<div class="form-group">
		  					<textarea class="form-control-plaintext pl-3 underline" placeholder="Deskripsi" name="deskripsi"></textarea>
						</div>
					</div>
				</div>
			</div>
