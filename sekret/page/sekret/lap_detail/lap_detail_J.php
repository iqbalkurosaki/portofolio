	...
	<div class="well sidebar-nav">
		...
		<div class="table-responsive">
			...
			<tr class="success">
			 ...
			 <tr class="active"><th>No</th><th>Nis</th><th>Nama</th><th>Tempat, Tanggal Lahir</th><th>No Hp</th><th>Profesi</th><th>Nama Orang Tua</th><th>Nama Wali</th><th>Telp. Rumah</th><th>Alamat</th><th>Tanggal Masuk</th><th>Kamar</th><th>Kelas</th><th>Status</th></tr>
           
           	<?php
           
            
            $sql = "SELECT * FROM t_santri where id_komplek LIKE '10%'";
            $no = 1;
            ...
            while (!$result->EOF) {
				...
				echo '<tr><td>'.$no.'</td><td>'.$nis.'</td><td>'.$nama.'</td><td>'.$tempat_l.','.$tgl_l.'</td><td>'.$no_hp.'</td><td> '.$jurusan.'.'.$nama_institusi.'</td><td>Ayah : '.$ayah.' Ibu:'.$ibu.'</td><td>'.$wali.'</td><td>'.$tlp_rumah.'</td><td>'.$alamat.','.$nama_kota.'. '.$nama_provinsi.'. Kode Pos :'.$kode_pos.'.</td><td>'.$tgl_masuk.'</td><td>'.$komplek.'</td><td>'.$kelas.'</td><td>'.$status.'</td>
				</tr>'; 
				$no++;
