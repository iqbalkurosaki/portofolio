...
while (!$result1->EOF) {
	...
	$object->setActiveSheetIndex(0)
	->setCellValue('A'.$coordinatRow, 'NAMA');
	$object->setActiveSheetIndex(0)
	->setCellValue('B'.$coordinatRow, 'ALAMAT');
	$object->setActiveSheetIndex(0)
	->setCellValue('C'.$coordinatRow, 'TTL');
	$object->setActiveSheetIndex(0)
	->setCellValue('D'.$coordinatRow, 'KAMAR');
	$object->setActiveSheetIndex(0)
	->setCellValue('E'.$coordinatRow, 'PENDIDIKAN/JURUSAN');
	$object->setActiveSheetIndex(0)
	->setCellValue('F'.$coordinatRow, 'STATUS');
	$object->getActiveSheet()->getStyle("A".($coordinatKomplek-1).":F".($coordinatRow))->applyFromArray($styleArrayCenter);
	$coordinatRow++;
	...
	while (!$result2->EOF) {
		...
		$result3=$db->Execute($sql3);
			...
			while (!$result3->EOF) {
				...
				$ttl = $result3->fields["tempat_lahir"].", ".date("d-m-Y", strtotime($result3->fields["tgl_lahir"]));
				$kamar = $result3->fields["nama_komplek"];
                $institusi = $result3->fields["id_institusi"];
                $jurusan = $result3->fields["jurusan"];
                $sql4 = "select * from profesi, institusi where profesi.id_profesi = institusi.id_profesi and institusi.id_institusi = $institusi";
			    $result4 = $db->Execute($sql4);
			    $nama_profesi = $result4->fields["nama_profesi"];
				$nama_institusi = $result4->fields["nama_institusi"];
				if ($nama_profesi == "Mahasiswa" || $nama_profesi == "Siswa") {
					$pendidikan = $nama_institusi.".".$jurusan;
				} else if ($nama_profesi == "Santri") {
					$pendidikan = $nama_profesi.".".$nama_institusi;
				} else {
					$pendidikan = $nama_profesi;
				}
				$status = $result3->fields["status_santri"];
				...
				->setCellValue('D'.$coordinatRow, $kamar);
				$object->setActiveSheetIndex(0)
				->setCellValue('E'.$coordinatRow, $pendidikan);
				$object->setActiveSheetIndex(0)
				->setCellValue('F'.$coordinatRow, $status);
	...
	$coordinatRow++;
	$object->getActiveSheet()->getStyle("A".$coordinatKomplek.":F".($coordinatRow-2))->applyFromArray($styleBorderArray);
	...
	$result1->MoveNext();
		...
		// Redirect output to a client’s web browser (Excel2007)
			...
			header('Content-Disposition: attachment;filename="Import Data Santri.xlsx";');
