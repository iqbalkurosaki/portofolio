...
$sql1="select * from komplek WHERE id_komplek NOT IN (20) ORDER BY `komplek`.`nama_komplek` ASC";
...
while (!$result1->EOF) {
	...
	$object->getActiveSheet()->mergeCells("A".$coordinatRow.":O".$coordinatRow);
	$coordinatRow++;
	...
	$object->setActiveSheetIndex(0)
	->setCellValue('C'.$coordinatRow, 'NIK');
	$object->setActiveSheetIndex(0)
	->setCellValue('D'.$coordinatRow, 'Nama');
	$object->setActiveSheetIndex(0)
	->setCellValue('E'.$coordinatRow, 'Kamar');
	$object->setActiveSheetIndex(0)
	->setCellValue('F'.$coordinatRow, 'No HP');
	$object->setActiveSheetIndex(0)
	->setCellValue('G'.$coordinatRow, 'Tempat, Tangal Lahir');
	$object->setActiveSheetIndex(0)
	->setCellValue('H'.$coordinatRow, 'Alamat');
	$object->setActiveSheetIndex(0)
	->setCellValue('I'.$coordinatRow, 'Profesi');
	$object->setActiveSheetIndex(0)
	->setCellValue('J'.$coordinatRow, 'Nama Orang Tua');
	$object->setActiveSheetIndex(0)
	->setCellValue('K'.$coordinatRow, 'Nama Wali');
	$object->setActiveSheetIndex(0)
	->setCellValue('L'.$coordinatRow, 'Telp Orang Tua');
	$object->setActiveSheetIndex(0)
	->setCellValue('M'.$coordinatRow, 'Tangal Masuk');
	$object->setActiveSheetIndex(0)
	->setCellValue('N'.$coordinatRow, 'Kelas');
	$object->setActiveSheetIndex(0)
	->setCellValue('O'.$coordinatRow, 'Status');
	
	$object->getActiveSheet()->getStyle("A".($coordinatKomplek-1).":O".($coordinatRow))->applyFromArray($styleArrayCenter);
	$coordinatRow++;
		...
		$sql = "SELECT * FROM t_santri where id_komplek = '".$id_komplek."' and id_status NOT IN ('B', 'KMP') ORDER BY nis";
		// echo $sql3;
		$result=$db->Execute($sql);
			...
			while (!$result->EOF) {
				...
				$nama = decode($result->fields["nama"]);
            	$tempat_l = decode($result->fields["tempat_lahir"]);
				...
				$ayah=decode($result->fields["ayah"]);
				$ibu=decode($result->fields["ibu"]);
				$nama_ortu='Nama Ayah : '.$ayah. ', Ibu : '.$ibu;
				
				$wali=decode($result->fields["wali"]);
				...
				$object->setActiveSheetIndex(0)
				->setCellValueExplicit('B'.$coordinatRow, $nis, PHPExcel_Cell_DataType::TYPE_STRING);
				$object->setActiveSheetIndex(0)
				->setCellValueExplicit('C'.$coordinatRow, $nik, PHPExcel_Cell_DataType::TYPE_STRING);
				$object->setActiveSheetIndex(0)
				->setCellValue('D'.$coordinatRow, $nama);
				$object->setActiveSheetIndex(0)
				->setCellValue('E'.$coordinatRow, $kamar);
				$object->setActiveSheetIndex(0)
				->setCellValueExplicit('F'.$coordinatRow, $no_hp, PHPExcel_Cell_DataType::TYPE_STRING);
				$object->setActiveSheetIndex(0)
				->setCellValue('G'.$coordinatRow, $ttl);
				$object->setActiveSheetIndex(0)
				->setCellValue('H'.$coordinatRow, $alamat_lengkap);
				$object->setActiveSheetIndex(0)
				->setCellValue('I'.$coordinatRow, $profesi2);
				$object->setActiveSheetIndex(0)
				->setCellValue('J'.$coordinatRow, $nama_ortu);
				$object->setActiveSheetIndex(0)
				->setCellValue('K'.$coordinatRow, $wali);
				$object->setActiveSheetIndex(0)
				->setCellValueExplicit('L'.$coordinatRow, $tlp_rumah, PHPExcel_Cell_DataType::TYPE_STRING);
				$object->setActiveSheetIndex(0)
				->setCellValue('M'.$coordinatRow, $tgl_masuk);
				$object->setActiveSheetIndex(0)
				->setCellValue('N'.$coordinatRow, $kelas);
				$object->setActiveSheetIndex(0)
				->setCellValue('O'.$coordinatRow, $status);
				...
				//set number datatype STRING
				// $object->setActiveSheetIndex(0)
				// ->getCell('B'.$coordinatRow)->setDataType('s');
				// $object->setActiveSheetIndex(0)
				// ->getCell('C'.$coordinatRow)->setDataType('s');
				// $object->setActiveSheetIndex(0)
				// ->getCell('F'.$coordinatRow)->setDataType('s');
				// $object->setActiveSheetIndex(0)
				// ->getCell(M.$coordinatRow)->setDataType('s');
	...
	$object->getActiveSheet()->getStyle("A".$coordinatKomplek.":O".($coordinatRow-1))->applyFromArray($styleBorderArray);
	...
	$result1->MoveNext();
		...
		// Redirect output to a client’s web browser (Excel2007)
			...
			header('Content-Disposition: attachment;filename="Data Santri PPMH.xlsx";');
