...
$sql1="select * from komplek WHERE id_komplek NOT like 20 ORDER BY `komplek`.`nama_komplek` ASC";
$result1 = $db->Execute($sql1);
while (!$result1->EOF) {
	$id_kelas = $result1->fields["id_komplek"];
	$nama_kelas = $result1->fields["nama_komplek"];
	$object->setActiveSheetIndex(0)
	...
	$time = date("Y-m-d H:i:s", strtotime("2021-01-01 00:00:00"));
		...
		// echo $time;and Year(tgl_masuk) >= $year
		...
		$sql = "SELECT * FROM t_santri where id_komplek = '".$id_kelas."' and last_update > '".$time."' and id_status = 'SB' and Year(tgl_masuk) >= $year ORDER BY id_kelas";
		// echo $sql3;
		$result=$db->Execute($sql);

			while (!$result->EOF) {
				$nis= decode($result->fields["nis"]);
				...
				$jurusan=decode($result->fields["jurusan"]);
				...
				$alamat = decode($result->fields["alamat"]);
	...
	$result1->MoveNext();
		...
		// Redirect output to a client’s web browser (Excel2007)
			...
			header('Content-Disposition: attachment;filename="Data Santri Baru PPMH.xlsx";');
