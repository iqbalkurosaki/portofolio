...
if (isset($_POST['beli'])) {
	...
	$id_reksadana_tujuan = $_POST['id_reksadana_tujuan'];
	$id_reksadana = $_POST['id_reksadana'];
	$unit = $_POST['up'];
	$id_status_transaksi=3;

	$stmt2 = $db->prepare("INSERT INTO transaksi(id, id_jenis_transaksi, waktu, id_klien, id_reksadana, id_reksadana_tujuan, unit, id_status_transaksi) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
	$stmt2->bindParam(1, $id);
	$stmt2->bindParam(2, $id_jenis_transaksi);
	$stmt2->bindParam(3, $waktu);
	$stmt2->bindParam(4, $id_klien);
	$stmt2->bindParam(5, $id_reksadana);
	$stmt2->bindParam(6, $id_reksadana_tujuan);
	$stmt2->bindParam(7, $unit);
	$stmt2->bindParam(8, $id_status_transaksi);
	...
	<div class="container" >
		...
		<div class="row" style="padding: 0px 20px">
			...
			<form class="form-horizontal" action="" method="post">
				...
				<div class="form-group">
				  <label class="control-label col-sm-4" for="nab">Pilihan Reksadana :</label>
				  <div class="col-sm-8">
				    <select class="form-control" id="id_reksadana_tujuan" name="id_reksadana_tujuan" required="">
				    <?php 
				    	$stmt2=$db->prepare("SELECT * FROM reksadana WHERE reksadana.id NOT IN (SELECT reksadana.id FROM reksadana INNER JOIN up_klien ON up_klien.id_reksadana = reksadana.id WHERE up_klien.id_klien = ? AND up_klien.up > 0 )");
						$stmt2->bindParam(1, $_SESSION['id']);
				    	$stmt2->execute();
				    	
				    	while ($res2=$stmt2->fetch()) {
				    		?><option value=<?php echo $res2['id']; ?> > <?php  echo $res2['reksadana']; ?></option> <?php
				    	}
				     ?>
				 	</select>
				  </div>
