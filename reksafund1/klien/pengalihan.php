...
?>
		    <ul class="nav navbar-nav text-center" id="demo">
		      ...
		      <li class="active" ><a href="#"><i class="glyphicon glyphicon-share"></i> PENGALIHAN</a></li>
		    </ul>
<?php
		}
?>
	    	...
	    	while ($res = $stmt->fetch()){
				echo "
					<tr>
						<td>".$res[1]."</td>";
				if (!isset($_SESSION['akses'])) { ?>
					<td><button class="btn btn-success" onclick="window.location.href='login_page.php'">Alihkan</button></td>
	<?php
				} elseif ($_SESSION['akses']=='klien') { ?>
					<td><button class="btn btn-success" onclick="window.location.href='alih_reksadana.php?r=<?php echo $res[0]; ?>'">Alihkan</button></td>
	<?php
				}
				echo	"<td>$res[2]</td>
						<td>$res[3]</td>
	    ...
	    ?> <div class="jumbotron text-center"><h1 style="color: #D7D2D2">Tidak Ada Reksadana Yang Dapat Dialihkan</h1> </div>
	     <?php  
	} ?>
    </tbody>
  </table>
