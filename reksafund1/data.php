    	...
    	while ($res = $stmt->fetch()){
			echo "
				<tr>
					<td>".$res[1]."</td>";
			if (!isset($_SESSION['akses'])) { ?>
				<td><button class="btn btn-success" onclick="window.location.href='login_page.php'"><i class='glyphicon glyphicon-grain'></i> Beli</button></td>
<?php
			} elseif ($_SESSION['akses']=='klien') { ?>
				<td><button class="btn btn-success" onclick="window.location.href='klien/beli_reksadana.php?r=<?php echo $res[0]; ?>'"><i class='glyphicon glyphicon-grain'></i> Beli</button></td>
<?php
			}
			echo	"<td>".$res[2]."</td>
					<td>".$res[3]."</td>
					...
					<td>".$res[7]."</td>
					<td>".$res[8]."</td>
					<td><i style='color:orange' class='glyphicon glyphicon-star'></i> ".$res[9]."</td>";
?>
					<td><button class="btn btn-info" onclick="window.location.href='perkembangan.php?r=<?php echo $res[0] ?>'"><i class='glyphicon glyphicon-stats'></i> Lihat</button></td>
				</tr>
<?php
			}
		} else { ?>
